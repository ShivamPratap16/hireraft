...
function StatCard({ icon: Icon, label, value, color, sub }: {
  ...
  return (
    <div className="glass rounded-2xl p-5 hover:border-brand-500/20 transition-all group">
      ...
      <p className="text-3xl font-bold text-[var(--text-primary)]">{typeof value === 'number' ? value.toLocaleString() : value}</p>
...
export default function AdminDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[var(--text-primary)]">Platform Overview</h1>
        <p className="text-[var(--text-muted)] mt-1">Real-time metrics across all HireRaft users</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Users} label="Total Users" value={stats?.total_users ?? 0} color="bg-brand-500/15 text-brand-400" />
        <StatCard icon={FileText} label="Applications" value={stats?.total_applications ?? 0} color="bg-blue-500/15 text-blue-500" />
        <StatCard icon={CheckCircle} label="Bot Runs" value={stats?.total_runs ?? 0} color="bg-green-500/15 text-green-500" />
        <StatCard icon={Activity} label="Active (7d)" value={stats?.active_users_7d ?? 0} color="bg-purple-500/15 text-purple-500" />
      </div>
      ...
      <div className="glass rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between">
          <h2 className="text-sm font-semibold text-[var(--text-primary)] uppercase tracking-wider">Live Activity Feed</h2>
          ...
          {activity?.items?.length ? activity.items.map((e: any) => (
            <div key={e.id} className="px-6 py-3 flex items-center gap-4 hover:bg-[var(--surface-2)] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[var(--text-secondary)] truncate">{e.message}</p>
                <p className="text-[10px] text-[var(--text-muted)] mt-0.5">{e.platform} · user:{e.user_id?.slice(-6)}</p>
              </div>
            ...
            <div className="px-6 py-12 text-center text-[var(--text-muted)]">No activity yet. Bot runs will appear here in real-time.</div>
          )}
        </div>
      </div>
    </div>
